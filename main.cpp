//Some apps require special strategy (for example in file explorer. if you want to rename a folder, when you disable mouse the focus lefts on another window * current fix is to press alt again to move focus back)
...
struct UserSettings
{
	...
	int MouseAccelerationMultiplier = 3;
...
struct CustomSaves
{
	...
	static void loadSettings(const std::string& filename, UserSettings& settings)
	{
		...
		// Parse an int safely: keep the current value on garbage, never go below minVal
		auto parseInt = [](const std::string& val, int fallback, int minVal) {
			try {
				int v = std::stoi(val);
		...
		setters["baseMouseSpeed"] = [&](const std::string& val) { settings.baseMouseSpeed = parseInt(val, settings.baseMouseSpeed, 1); };
		setters["MouseAccelerationMultiplier"] = [&](const std::string& val) { settings.MouseAccelerationMultiplier = parseInt(val, settings.MouseAccelerationMultiplier, 1); };
		setters["baseScrollSpeed"] = [&](const std::string& val) { settings.baseScrollSpeed = parseInt(val, settings.baseScrollSpeed, 1); };
		setters["ScrollAccelerationDivider"] = [&](const std::string& val) { settings.ScrollAccelerationDivider = parseInt(val, settings.ScrollAccelerationDivider, 1); };
...
void InputLoop()
{
	...
	while (true)
	{
		// Failsafe: Pause/Break is never suppressed, so it always works.
		// Force everything OFF if active ever gets stuck (e.g. a CapsLock
		// key-up missed because of a UAC prompt / lock screen / desktop switch).
		...
		if (active)
		{
			...
			if ((GetAsyncKeyState(VK_CONTROL) & 0x8000) ||Alt_pressed) { 
				...
				if (US_Profile.ScrollAccelerationDivider != 0) ds /= US_Profile.ScrollAccelerationDivider;
			}
...
void AddAppStartup()
{
    // If we're already registered for startup, don't nag on every launch.
    HKEY hCheck;
    if (RegOpenKeyExW(HKEY_CURRENT_USER,
                      L"Software\\Microsoft\\Windows\\CurrentVersion\\Run",
                      0, KEY_QUERY_VALUE, &hCheck) == ERROR_SUCCESS)
    {
        bool exists = (RegQueryValueExW(hCheck, L"MyApp", nullptr, nullptr, nullptr, nullptr) == ERROR_SUCCESS);
        RegCloseKey(hCheck);
        if (exists) return;
    }
...
int WINAPI WinMain(_In_ HINSTANCE, _In_opt_ HINSTANCE, _In_ LPSTR, _In_ int)
{
	...
	if (!keyboardHook)
	{
		// std::cout is a no-op in a /SUBSYSTEM:WINDOWS app (no console),
		// so surface this with a message box instead of failing silently.
		MessageBoxW(nullptr, L"Failed to install the keyboard hook.", L"Error", MB_ICONERROR);
		return 1;
	}
